...
const getQuestionsQuizStudent = asyncHandler(async (req, res, next) => {
  const { idQuiz } = req.params;
  let total = 0;
  const [result] = await Question.findQuestionsByQuiz(idQuiz);
  await Promise.all(
    result.map(async (item, index) => {
      total += item.note;
      const [options] = await Question.findOptions(item.id);
      result[index].options = options.map((option) => option.optionText);
      result[index].optionsIds = options.map((value) => value.id);
    })
  );
  res.status(200).json({ questions: result, scoreTotal: total });
