...
const forgotPasswordController = asyncHandler(async (req, res, next) => {
    ...
    res.status(200).json({ message: 'Reset instructions sent to your email',token:token });
});
...
const resetPasswordController = asyncHandler(async (req, res, next) => {
	...
	console.log(role)
        ...
        if (role === 'student') {
            await Student.updatePassword(data.id, newPassword);
        } else if (role === 'teacher') {
            await Teacher.updatePassword(data.id, newPassword);
        } else if (user.role === 'admin') {
            await Admin.updatePassword(data.id, newPassword);
		}
...
module.exports = {signInController , verifyUserAccountCtrl,logoutController,forgotPasswordController,resetPasswordController}
