...
class AssignmentResult {
  ...
  static findByAssignmentAndStudent(assignmentId, studentId) {
    return db.execute(
      "SELECT * FROM assignmentresult WHERE assignmentId = ? AND studentId = ?",
      [assignmentId, studentId]
    );
  }

  static updateStatus(assignmentId, studentId, status) {
    return db.execute(
      "UPDATE assignmentresult SET status = ? WHERE assignmentId = ? AND studentId = ?",
