...
import { AdminNav, TeacherTable } from "../../components";
...
const Teacher = () => {
  ...
  const handleInsertOneTeacher = async (e) => {
    e.preventDefault();
    setErrorMsg();
    setSuccessMsg();
    try {
      const response = await admin.insertOneTeacher({
        fullName,
        email,
        password,
      });
  ...
  const handleInsertTeachers = async () => {
    try {
      const formData = new FormData();
      formData.append("file", file);
      const response = await admin.insertTeachers(formData);
      ...
      {showModal && (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-[#00194F80] bg-opacity-50 z-20">
          <div className="bg-white p-8 rounded-md w-[90%] md:w-[40%]">
            ...
            <form onSubmit={handleInsertOneTeacher}>
              <div className="mb-4">
                <label
                  htmlFor="name"
